...
use crate::error::SweepSolError;
use crate::planner::SweepPlan;
use crate::sol::{build_and_sign_sweep_sol_transaction, SweepSolParams, MIN_REMAINING_LAMPORTS};
...
pub fn execute_sweep_plan(
    ...
    // ---------------------------------------------------------------------
    // 1) Fetch the *current* on-chain balance of P.
    //    We do not blindly trust `plan.amount`, since the account might
    //    have changed between planning and execution.
    // ---------------------------------------------------------------------
    let balance = rpc.get_balance(&plan.one_time_pubkey)?;

    if balance <= MIN_REMAINING_LAMPORTS {
        return Err(SweepExecutorError::NothingToSweep {
            one_time_pubkey: plan.one_time_pubkey.to_string(),
            balance,
            min_remaining: MIN_REMAINING_LAMPORTS,
        });
    }

    let sweep_amount = balance.saturating_sub(MIN_REMAINING_LAMPORTS);
